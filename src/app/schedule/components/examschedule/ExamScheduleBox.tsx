...
const ExamSchedule: React.FC<ExamScheduleProps> = ({ item, selectedSection, examType }) => {
    if (!item || !item.detail) {
    ...
    const teachTableForSelectedSection = detail?.teach_table?.find(
        (teachTable: TeachTableDto) => teachTable.section === selectedSection
    );
    ...
    const examStartTime = examType === 'midterm' ? midterm_exam_time_start : final_exam_time_start;
